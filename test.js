...
const premiumData = JSON.parse(fs.readFileSync('./config/premium.json'));
...
module.exports = new Command({
  ...
  run: async ({ client, interaction, args, prefix }) => {
    ...
    if (!premiumData[userId] || !premiumData[userId].isPremium) {
      return client.embed(
        interaction,
        `** 🚫 Sorry, this feature is only available to premium subscribers. **`
      );
    }
      ...
      default:
        {
          return client.embed(
            interaction,
            `** ❌ Invalid filter! Available filters: 8d, bassboost, ... (list all available filters here) **`
          );
        }
        break;
    }
...
async function setFilter(client, interaction, player, filter) {
  ...
  if (!channel) {
    ...
    return interaction.followUp(
      `** 🚫 You are Not DJ and also not Song Requester **`
    );
  } else {
    await queue.setFilter(filter);
    return client.embed(
      interaction,
      `** ✅ Added \`${filter}\` Filter in Song **`
    );
  }
}
